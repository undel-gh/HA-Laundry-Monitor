...
Heating context that affects future safety decisions is different from confirmation-path metadata. A latched `heating_detected` fact survives safe restart recovery. A confirmed `not_seen` conclusion may also be restored when the persisted detector persistence version and all heating-detector parameters exactly match the current detector. Source freshness, `heating_active`, and partial heater-confirmation coverage are never restored. If that compatibility proof is unavailable, recovery is conservative: heating returns to `unknown`.

The runtime also persists timestamps of vibration rising-edge evidence from the active spin window. On recovery, the Spin Detector re-prunes those timestamps against the current configured window and seeds its edge baseline from the current vibration state, preventing an already-ON startup state from becoming synthetic evidence. Electrical rolling-window samples remain non-persistent and must reacquire freshness from real post-restart updates.
